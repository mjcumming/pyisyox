...
class ISY:
    ...
    def __init__(
        ...
        Args:
            ...
            use_websocket: ``True`` (default) opens a WebSocket event
                stream; ``False`` falls back to TCP.
            args: Optional CLI args namespace.
            max_concurrent: Optional override for the outbound HTTP
                concurrency cap (forwarded to :class:`Connection`).
                ``None`` (default) uses hub-platform auto-detection.
                Set explicitly (e.g. ``1`` for old/slow hardware,
                ``5`` for a healthy Polisy on a fast LAN) to override.
        """
        ...
        self.conn = Connection(connection_info, args, max_concurrent=max_concurrent)
